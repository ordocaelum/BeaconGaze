...
/* planets are positioned + animated purely via inline style.transform
   (never a CSS class transform — a class transform would REPLACE the
   positioning translate and teleport the glyph to the 0,0 corner) */
.planet{cursor:pointer;transition:transform .75s cubic-bezier(0.16,1,0.3,1),opacity .4s;
  will-change:transform}
.planet text{font-size:30px;fill:var(--silver);text-anchor:middle;dominant-baseline:central;
  font-family:'Segoe UI Symbol','Noto Sans Symbols 2',serif;
  filter:drop-shadow(0 0 6px rgba(139,108,240,.55));transition:filter .4s}
.planet:hover text{filter:drop-shadow(0 0 12px rgba(139,108,240,.9))}
...
.housenum{font-size:13px;fill:var(--dim);text-anchor:middle;dominant-baseline:central;
  letter-spacing:.1em}
/* examine card: centered beneath the summoned symbol */
#card{position:fixed;left:50%;top:calc(50% + 110px);max-width:340px;width:calc(100vw - 44px);
  ...
  opacity:0;pointer-events:none;transform:translate(-50%,26px);
  transition:transform .6s cubic-bezier(0.16,1,0.3,1),opacity .5s;z-index:5;
  max-height:calc(50vh - 130px);overflow-y:auto}
#card.show{opacity:1;pointer-events:auto;transform:translate(-50%,0)}
...
DATA.planets.forEach(p => {
  ...
  g._home = \`translate(\${x}px, \${y}px)\`;
  g.style.transform = g._home;
...
function focusEl(target) {
  ...
  planetEls.forEach(n => {
    ...
    /* summoned symbol glides to the wheel's center, enlarged;
       everyone else returns home */
    n.style.transform = on
      ? "translate(500px, 500px) scale(2.2)"
      : n._home;
  });
...
function clearFocus() {
  ...
  planetEls.forEach(n => {
    n.style.transform = n._home;
    n.setAttribute("aria-pressed", "false");
  });
  card.classList.remove("show");
}
/* click / tap to examine; same element again (or backdrop, or Esc)
   releases it — no hover hijacking */
function toggleEl(n) { (SELECTED === n) ? clearFocus() : focusEl(n); }
[...planetEls, ...aspectEls].forEach(n => {
  n.addEventListener("click", e => { e.stopPropagation(); toggleEl(n); });
  n.addEventListener("keydown", e => {
    if (e.key === "Enter" || e.key === " ") { e.preventDefault(); toggleEl(n); }
  });
});
